...
- 意图缓存 Collection `feeding_intents` 在运行时按确认成功后写入，启动默认不清空
- 若测试数据污染了意图缓存：临时设置 `CLEAR_FEEDING_INTENTS_ON_STARTUP=true` 后重启，仅清空 `feeding_intents`（不得动 `mother_baby_knowledge`）；完成后务必改回 `false`
