...
对线性步骤：先让调用方 yield thinking，再 await 节点并合并 state。

设计思路：
1. run_linear_steps_with_thinking：线性步骤表，逐步 yield (node_name, message)
2. 每步 await 后 asyncio.sleep(0)，便于事件循环刷出 SSE
"""
...
async def run_linear_steps_with_thinking(
    ...
    2. 本函数在 yield 之后才 await 节点（生成器协议：consumer 处理完 yield 值后才 resume）
    3. 因此「先输出思考再执行」成立

    Args:
        state: 可变状态字典（就地 update）
        steps: [(节点名, async 节点函数), ...]
