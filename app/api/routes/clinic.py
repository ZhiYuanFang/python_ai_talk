...
业务说明：
提供 /v1/clinic 与 /v1/clinic/stream：家长续聊，按 device_no 的 Python Redis 会话。
数据准备由 clinic_graph 统一编排（画像/月龄 + 按需历史）；不再走 tip / Q&A / 通识飞轮。
"""
...
async def clinic_stream(request: ClinicRequest):
    ...
    业务逻辑：
    clinic_graph 编排准备并推送逐步 thinking；流式返回回答。
    """
    logger.info(
