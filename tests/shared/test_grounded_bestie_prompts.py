"""grounded-bestie + dialogue-hooks：有据点名 / 引导收尾 / 同月龄 / 约 80 字。"""
...
def test_clinic_system_has_grounded_bestie_rules():
    ...
    assert "约 80 字" in text
    assert "约 50 字" not in text
    assert "不要做疾病诊断" in text
    assert "对话感" in text
    assert "引导式" in text
    assert "同月龄代入" in text
    assert "先念清时间" in text or "先答准事实" in text
...
def test_clinic_user_history_only_requires_cite():
    ...
    assert "引导式话题收尾" in msg
    assert "约 80 字" in msg
    assert "同月龄代入" in msg
...
def test_tip_prompts_aligned():
    ...
    assert "约 80 字" in sys_text
    assert "约 50 字" not in sys_text
    assert "禁止编造" in sys_text
    assert "对话感" in sys_text
    assert "同月龄代入" in sys_text
    ...
    assert "引导式话题收尾" in with_hist
    assert "大约80字" in with_hist or "约 80 字" in with_hist
