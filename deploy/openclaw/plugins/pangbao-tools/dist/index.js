...
function apiTokenFromCtx(ctx) {
    const envTok = (process.env.PANGBAO_API_TOKEN ?? "").trim();
    if (envTok)
        return envTok;
    // Spike：OpenClaw 若把入站头挂到 ctx.headers 则可取；否则依赖进程环境 PANGBAO_API_TOKEN
    const headers = ctx.headers;
    if (headers?.[API_TOKEN_HEADER])
        return headers[API_TOKEN_HEADER];
    if (headers?.["X-Pangbao-Api-Token"])
        return headers["X-Pangbao-Api-Token"];
    return "";
}
async function httpJson(method, url, opts) {
    ...
    if (opts.apiToken)
        headers[API_TOKEN_HEADER] = opts.apiToken;
