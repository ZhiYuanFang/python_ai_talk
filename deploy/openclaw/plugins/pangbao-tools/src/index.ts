...
const ConfigSchema = Type.Object({
  historyBaseUrl: Type.Optional(
    Type.String({ description: "Go 历史 API 基址，如 http://go-ai-talk:8001" }),
  ),
  historyToken: Type.Optional(
    Type.String({ description: "可选 Bearer，调 Go 内网接口" }),
  ),
...
function historyBase(config: PluginConfig): string {
...
function historyToken(config: PluginConfig): string {
  ...
  tools: (tool) => [
    ...
    tool({
      ...
      async execute(args, config, ctx) {
        ...
        return httpJson(
          "POST",
          `${historyBase(config)}/device/history/api/event/end-latest`,
          { body: args, token: historyToken(config), signal: ctx.signal },
        );
      },
    }),
    // —— 只读历史 ——
    tool({
      ...
      async execute(args, config, ctx) {
        ctx.signal?.throwIfAborted();
        const url =
          `${historyBase(config)}/device/history/api/filter` +
          qs({
    ...
    tool({
      ...
      async execute(args, config, ctx) {
        ctx.signal?.throwIfAborted();
        const url =
          `${historyBase(config)}/device/history/api/list` +
          qs({
            deviceNo: args.deviceNo,
            page: args.page,
    ...
    tool({
      ...
      async execute(args, config, ctx) {
        ctx.signal?.throwIfAborted();
        const url =
          `${historyBase(config)}/device/history/api/birthday` +
      ...
      async execute(args, config, ctx) {
        ...
        try {
          return await httpJson(
            "POST",
            `${toolsBase(config)}/tools/flywheel/clinic/retrieve`,
            { body: args, signal: ctx.signal },
          );
        } catch {
    ...
    tool({
      ...
      async execute(args, config, ctx) {
        ...
        return httpJson(
          "POST",
          `${toolsBase(config)}/tools/clinic/judge_implicit_acceptance`,
          { body: args, signal: ctx.signal },
        );
      },
    }),
