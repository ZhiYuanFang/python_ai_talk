...
系统 SHALL 使用 LangGraph StateGraph 构建意图分析流程图（intent_graph），包含意图缓存匹配、分类前进行中计时探针、意图分类、**分类后备注反查**（仅当存在字典未定名称时由条件边进入）、条件路由、批量历史 CRUD 与查记录模板播报。图 MUST NOT 在分类前做备注探针或注入备注摘要。图 MUST NOT 包含 `match_event_by_vector` 节点，MUST NOT 包含 `call_clinic_agent` 节点，MUST NOT 包含 `judge_data_requirement` 或历史答题 LLM 节点，MUST NOT 从 `feeding` 导入 `clinic` 图。

分类后的条件路由（`route_after_classify` 或等价）MUST：当顶层或 `events[]` 存在「`event_name` 非空且 `event_id` 为空」的槽位时，进入备注反查节点；否则 MUST NOT 进入该节点，并 SHALL 按与反查完成后相同的规则路由到确认结束、模板查记录或批量 CRUD。备注反查节点 SHALL 只执行反查与写回/消歧/无法识别，MUST NOT 依赖「无槽位空返回」作为跳过反查的主路径。
...
- **AND** 图 SHALL 注册分类后备注反查步骤（独立节点或等价），且仅由分类后条件边在存在未定名称时进入
- **AND** 图 SHALL NOT 在分类前注册仅用于备注 OOV 探针的步骤，SHALL NOT 将备注探针摘要写入分类系统提示
- **AND** 图 SHALL 包含条件边，根据缓存命中、未定名称、确认需求与 `op`/`target_type` 路由
- **AND** 图 SHALL NOT 注册 `match_event_by_vector`、`call_clinic_agent` 或历史答题 LLM

#### Scenario: 无未定名称时跳过备注反查节点
- **WHEN** 分类完成且顶层与 `events[]` 均无「有 event_name、无 event_id」的槽位（含闲聊/退出、字典已写满 id）
- **THEN** 图 SHALL NOT 进入备注反查节点
- **AND** SHALL 按确认需求与 `op`/`target_type` 直接路由到 END、模板查记录或批量 CRUD（与反查完成后相同规则）

#### Scenario: 有未定名称时进入备注反查
- **WHEN** 分类完成且存在「有 event_name、无 event_id」的槽位
- **THEN** 图 SHALL 进入备注反查节点再按确认/op 继续路由
