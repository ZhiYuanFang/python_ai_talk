...
当用户所说名称经字典匹配（含简称/进行态对应）仍无法落到表内事件时，系统 MUST NOT 将其当作新事件类型。系统 SHALL 在**意图分类完成之后**，由意图图条件路由仅在存在「有 `event_name`、无 `event_id`」槽位时进入备注反查步骤；该步骤将该名称作为 `remark` 调用兄弟仓 filter（本设备、建议近窗、小 limit、空 `eventIds`）做备注反查，按命中行的事件聚合，再写入字典 `event_id` 与 `remark_keyword`。无上述未定槽位时 MUST NOT 调用备注反查 HTTP。备注反查实现 MUST 专注反查与写回/消歧/无法识别，MUST NOT 以「无槽位空返回」作为跳过反查的主门控（主门控在分类后条件边）。系统 MUST NOT 在分类前做备注探针，MUST NOT 把备注探针摘要或原始历史行注入分类 prompt。系统 MUST NOT 用本地查询句正则决定是否反查或是否为 read。系统 MUST NOT 用常识将表外专名改写成某字典类别事件后再跳过反查（实现上：字典未命中才反查；提示亦禁止专名升格）。
...
- **AND** 因 AD 不在字典，SHALL 由分类后条件边进入备注反查
- **AND** SHALL NOT 直接建新事件或拉全量史
- **AND** SHALL NOT 在分类前因缺少「上一次」等字样而跳过反查路径的可用性（反查触发条件是字典未命中名称槽位，不是查询句式）

#### Scenario: 已定事件名不调备注反查
- **WHEN** 分类完成且所有相关槽位已有 `event_id`（或无事件名槽位）
- **THEN** 系统 MUST NOT 因本轮分类结果调用备注反查 filter
- **AND** SHALL 按确认与 `op` 继续后续路径
