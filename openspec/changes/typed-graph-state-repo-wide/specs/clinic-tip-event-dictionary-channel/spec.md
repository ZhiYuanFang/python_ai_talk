...
`ClinicState` SHALL 为 Pydantic 模型，并包含字段 `event_dictionary`（与意图 State 同名同义的事件字典列表），使 LangGraph 在图执行期间保留路由或嵌套调用写入的事件字典。
...
`TipState` SHALL 为 Pydantic 模型，并同样声明 `event_dictionary`，使 tip 图中依赖该字段的共享节点（含 `judge_data_requirement`）能读到路由注入值。
...
在路由已注入且缓存返回非空的前提下，clinic/tip 的「事件字典为空」告警 SHALL NOT 由 State 模型缺字段或补丁合并丢字段导致。
