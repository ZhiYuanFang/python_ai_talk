...
- **BREAKING（方案 B）**：意图路径 **SHALL NOT** 再做事件名向量匹配。删除 `match_event_by_vector`、`EventVectorStore`、Chroma `feeding_events` 及其字典同步（启动初始化 / `event_cache.sync_events`）。冷启动一律走分类 LLM；重复句只靠 `feeding_intents`。知识库 `mother_baby_knowledge` 与 `source=user` Q&A **不删**。
...
- 分类 miss 后默认 **先确认再落库**。免确认仅限 **意图缓存高置信命中**（该句曾确认并落库成功）。多事件一律软确认。不得再以事件名向量高分免确认。
...
- `feeding-intent-vector-matching`：删除事件名向量匹配；不得再以 Top-1 事件名短路分类
- `feeding-events-vector-store`：拆除 `feeding_events` Collection、启动初始化与字典同步
- `langgraph-intent-graph`：缓存 →（可选备注探针）→ 分类 → 确认 | 批量执行 | 模板查记录；无 `match_event_by_vector`；移除 clinic / 历史答题 LLM
...
- **Python**：意图图、分类提示、飞轮删除、拆除 `feeding_events` / `match_event_by_vector`、意图缓存、批量写客户端、filter 备注、模板回执、`/intent` 同图、`POST /v1/clinic`、clinic/tip 拉史 unix 对齐
