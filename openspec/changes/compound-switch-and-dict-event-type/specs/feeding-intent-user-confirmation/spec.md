...
系统 SHALL 根据意图结果生成清晰的确认话术，包含事件名称和动作描述。查记录确认 MUST 点出字典事件名（叶子或父），MUST NOT 使用「该事件」等不含名称的句子。当 `events[]` 含多项或同时含结束与开始/记录时，确认话术 MUST 逐项点出字典事件名与对应动作（结束 / 开始 / 记录），MUST NOT 仅列举名称并写成「记录以下事件」。
